...
import With, { contextmanager, Use, ExitStack } from '../src/contextlib';
describe('ContextManager - General Tests', () => {
    test("The value passed to the context body is the return value of the cm's enter method", () => {
        ...
        // generator cms
        const gencm = contextmanager(function* (value) {
            yield value;
        })('Value');
        With(gencm, value => {
...
describe('Generator Context Managers', () => {
    test('contextmanagers must yield once and only once', () => {
        const nonYieldingGeneratorContextManager = contextmanager(function* () { });
        const multiYieldingGeneratorContextManager = contextmanager(function* () {
            yield 1;
            yield 2;
        });
        expect(() => {
            With(nonYieldingGeneratorContextManager(), () => { });
        }).toThrow('Generator did not yield!');
        expect(() => {
            With(multiYieldingGeneratorContextManager(), () => { });
...
describe('ExitStack', () => {
    test('An exitstack acts exactly like nested context managers', () => {
        ...
        const logB = jest.fn(console.log.bind(console, 'B'));
        const cmA = contextmanager(function* () {
            yield 'A';
            for (let a = 0; a <= 2; a++) {
                logA();
            }
        });
        const cmB = contextmanager(function* () {
            yield 'B';
            for (let b = 0; b <= 2; b++) {
                logB();
            }
    ...
    test('When multiple errors occur in an exitstack, the last error would be thrown', () => {
        const buggycm = contextmanager(function* (program) {
            try {
                yield program;
            }
            catch (error) {
...
describe('Use', () => {
    ...
    test('error', async () => {
        ...
        await (async () => {
            ...
            expect('didn\'t throw error').toEqual('threw error');
        })();
        expect(out).toStrictEqual([
            1,
            5,
